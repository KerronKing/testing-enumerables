...
RSpec.describe Enumerable do
  ...
  describe '#my_map' do
    it 'returns a new array as a result of calling a block once on each item in the original array (multiplication case)' do
      result = [2, 4, 6]
    ...
    it 'returns a new array as a result of calling a block once on each item in the original array (subtraction case)' do
      result = [-1, 0, 1]
      expect(arr.my_map { |x| x - 2 }).to(eql(result))
    end
  end
